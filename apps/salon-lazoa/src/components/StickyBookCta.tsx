...
export default function StickyBookCta({ t, onBook, hidden }: Props) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 md:hidden px-4 safe-bottom pt-3 pointer-events-none">
      <button
        onClick={onBook}
        className="pointer-events-auto w-full bg-ink text-paper rounded-full py-3.5 font-medium flex items-center justify-center gap-2 shadow-[0_10px_30px_-8px_rgba(0,0,0,0.4)]"
      >
        <Calendar size={18} />
        {t('sticky.book')}
      </button>

...
const usps = ['Geverifieerd op Werkspot', 'Met garantie', 'Snelle reactie', 'KvK-ingeschreven'];
          ...
          <div className="grid gap-8 md:grid-cols-[1fr_1.4fr] px-5 md:px-8 py-6 md:py-8">
            <aside className="hidden md:flex flex-col">
              <p className="text-stone text-base leading-relaxed">
                Vul je gegevens in. WhatsApp opent met je aanvraag. We reageren dezelfde dag.
              </p>
              <ul className="mt-6 space-y-3">
                {usps.map((u) => (
                  <li key={u} className="flex items-start gap-3">
                    <Check className="mt-0.5 h-5 w-5 flex-shrink-0 text-gold-deep" />
                    <span className="text-ink font-medium">{u}</span>
                  </li>
                ))}
              </ul>
            </aside>

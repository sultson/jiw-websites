...
import { Sparkles, Plus, Wrench, ChevronDown, Phone, MessageCircle } from 'lucide-react';
...
export default function Services({ lang, t }: Props) {
  const [open, setOpen] = useState<string | null>('behandelingen');
  ...
  function svcPrice(s: { price: number; priceDisplayNl?: string; priceDisplayEn?: string }) {
    const display = lang === 'en' ? s.priceDisplayEn : s.priceDisplayNl;
    return display ?? formatPrice(s.price);
  }
  ...
  return (
    <section id="behandelingen" className="py-20 md:py-28 bg-blush-soft/50">
      <div className="max-w-5xl mx-auto px-5 sm:px-8">
        ...
        <p className="mt-6 text-center text-sm text-espresso/60">{t('services.color')}</p>

        <div className="mt-10 text-center">
          <p className="text-espresso/70 max-w-md mx-auto leading-relaxed">{t('services.ctaLine')}</p>

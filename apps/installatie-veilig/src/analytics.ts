const SITE_URL = 'https://installatieveilig.nl';

export const MEASUREMENT_ID = 'G-MGZVJV1ELY';
const CONSENT_KEY = 'iv-analytics-consent-v1';
type Consent = 'granted' | 'denied';
type Context = {site_language: string; page_type: string};
declare global {
  interface Window {
    dataLayer?: unknown[];
...
let context: Context = {site_language: 'nl', page_type: 'landing'};
let currentPath = '/';
let lastPage = '';
let started = false;
...
function start() {
  ...
  script.id = 'iv-analytics'; script.async = true;
  script.src = `https://www.googletagmanager.com/gtag/js?id=${MEASUREMENT_ID}`;
  document.head.append(script);
}

export function trackPage(path = '/', details: Context = {site_language: 'nl', page_type: 'landing'}) {
  currentPath = path;
  context = details;
...
export function trackEvent(name: 'contact_click' | 'enquiry_click' | 'generate_lead', method?: 'whatsapp' | 'phone' | 'email' | 'form') {
  if (!allowed()) return;
  try {
    start();
...
export function setConsent(value: Consent) {
  ...
  window.dataLayer = [];
  document.getElementById('iv-analytics')?.remove();
...
export function trackContactLink(event: MouseEvent) {
  ...
  if (href.startsWith('tel:')) trackEvent('contact_click', 'phone');
    ...
    try {
      ...
      else if (url.origin === location.origin && url.hash === '#aanvraag') trackEvent('enquiry_click');

...
const serviceOptions = [
  { value: 'paint',    labelKey: 'intake.opt.paint' },
  { value: 'wall',     labelKey: 'intake.opt.wall' },
  { value: 'spray',    labelKey: 'intake.opt.spray' },
  { value: 'glass',    labelKey: 'intake.opt.glass' },
  { value: 'heritage', labelKey: 'intake.opt.heritage' },
  { value: 'other',    labelKey: 'intake.opt.other' },
];
    ...
    >
      ...
      >
        <div className="sticky top-0 z-10 bg-paper/95 backdrop-blur-sm border-b border-ink/10 px-5 sm:px-7 py-4 flex items-start justify-between">
          <div>
            <span className="kicker">{t('intake.kicker')}</span>
            <h2 id="intake-title" className="mt-1 font-display text-2xl sm:text-3xl leading-tight">
              {t('intake.title')}
            </h2>
          </div>
          <button
        ...
        {state === 'success' ? (
          ...
          <form ref={formRef} onSubmit={submit} className="px-5 sm:px-7 py-6 flex flex-col gap-5">
            <p className="text-sm text-ink/75 leading-relaxed">{t('intake.subtitle')}</p>
            ...
            <Field label={t('intake.field.photos')} optional t={t}>
              <FileDrop
                files={files}
                onAdd={addFiles}
                onRemove={removeFile}
  ...
  return (
    <label className="block">
      <span className="label">
        {label}
        {optional && t && (
          <span className="ml-1.5 normal-case tracking-normal text-ink/40 font-normal">
            ({t('intake.optional')})
          </span>
        )}
      </span>
    ...
    <div>
      <label
        className={`relative block border-2 border-dashed rounded-sm px-4 py-6 text-center cursor-pointer transition-colors ${
          dragOver ? 'border-orange bg-orange/5' : 'border-ink/20 bg-paper-soft hover:border-ink/35'
        }`}
      ...
      >
        ...
        <div className="text-sm font-semibold text-ink">{t('intake.dropzone.idle')}</div>
          ...
          {files.map((f, i) => (
            <li key={`${f.name}-${i}`} className="flex items-center justify-between gap-3 text-sm bg-paper-soft border border-ink/10 px-3 py-2 rounded-sm">
              <div className="flex items-center gap-2 min-w-0">
                <Check size={14} className="text-orange-deep shrink-0" />
...
function SuccessPanel({ t, onAgain }: { t: (k: string) => string; onAgain: () => void }) {
  return (
    <div className="px-5 sm:px-7 py-12 text-center flex flex-col items-center">
      <div className="w-14 h-14 rounded-full bg-orange/15 text-orange-deep flex items-center justify-center mb-5">
        <Check size={28} strokeWidth={2.5} />
      </div>
      <h3 className="font-display text-2xl text-ink">{t('intake.success.title')}</h3>
      <p className="mt-3 text-ink/75 max-w-md leading-relaxed">{t('intake.success.body')}</p>

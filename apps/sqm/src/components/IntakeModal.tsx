...
export default function IntakeModal({ open, onClose, t }: Props) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 bg-charcoal-ink/70 backdrop-blur-sm flex items-stretch sm:items-center justify-center sm:p-4"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        className="bg-cream w-full sm:max-w-2xl max-h-[100dvh] sm:max-h-[92vh] overflow-y-auto sm:rounded-sm shadow-2xl flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="sticky top-0 z-10 bg-cream/95 backdrop-blur-sm border-b border-charcoal/10 px-5 sm:px-7 py-4 flex items-start justify-between">
          <div>
          ...
          <button
            type="button"
            className="p-2 -mr-2 text-charcoal-ink/70 hover:text-charcoal-ink"
            onClick={onClose}
            aria-label="Close"
          >
            <X size={22} />
        ...
        {state === 'success' ? (
          ...
          <form ref={formRef} onSubmit={submit} className="px-5 sm:px-7 py-6 flex flex-col gap-5">
            <p className="text-sm text-charcoal/75 leading-relaxed">{t('ct.intro')}</p>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <Field label={t('ct.urgency')} optional t={t}>
                <select name="urgency" defaultValue="" className="field">
                  <option value="" disabled>{t('ct.urgencyPick')}</option>
                  <option value="planned">{t('ct.urgencyPlanned')}</option>
                  <option value="soon">{t('ct.urgencySoon')}</option>
                  <option value="urgent">{t('ct.urgencyUrgent')}</option>
                </select>
              </Field>
            </div>
            ...
            <label className="flex items-start gap-2.5 text-sm text-charcoal/75 leading-relaxed">
              <input type="checkbox" required className="mt-1 accent-orange" />
              <span>{t('ct.consent')}</span>
            </label>

            {state === 'error' && errorMsg && (
              <div className="flex items-start gap-2 text-sm text-orange-deep bg-orange/10 border border-orange/30 px-3 py-2.5 rounded-sm">
            ...
            <button
              ...
              className="btn-orange w-full sm:w-auto sm:self-start mt-2 disabled:opacity-60 disabled:cursor-wait"
            >
  ...
  return (
    <label className="block">
      <span className="label">
        {label}
        {optional && t && <span className="ml-1.5 normal-case tracking-normal text-charcoal/40 font-normal">({t('ct.optional')})</span>}
      </span>
      {children}
    </label>
  );
    ...
    <div>
      <label
        className={`relative block border-2 border-dashed rounded-sm px-4 py-6 text-center cursor-pointer transition-colors ${
          dragOver ? 'border-orange bg-orange/5' : 'border-charcoal/20 bg-cream-soft hover:border-charcoal/35'
        }`}
      ...
      >
        <Upload size={20} className="mx-auto mb-2 text-charcoal/60" />
        <div className="text-sm font-semibold text-charcoal-ink">
          {t('ct.attachments')}
        </div>
        <div className="text-xs text-charcoal/55 mt-1">{hint}</div>
        <input
          type="file"
          name="files"
      ...
      {files.length > 0 && (
        <ul className="mt-3 space-y-1.5">
          {files.map((f, i) => (
            <li key={`${f.name}-${i}`} className="flex items-center justify-between gap-3 text-sm bg-cream-soft border border-charcoal/10 px-3 py-2 rounded-sm">
              <div className="flex items-center gap-2 min-w-0">
                <Check size={14} className="text-orange shrink-0" />
                <span className="truncate font-medium text-charcoal-ink">{f.name}</span>
                <span className="text-charcoal/50 text-xs shrink-0">{(f.size / 1024 / 1024).toFixed(1)} MB</span>
              </div>
              <button
                type="button"
                onClick={() => onRemove(i)}
                className="p-1 text-charcoal/50 hover:text-charcoal-ink"
                aria-label="Remove"
              >
                <Trash2 size={14} />
              </button>
...
function SuccessPanel({ t, onAgain }: { t: (k: string) => string; onAgain: () => void }) {
  return (
    <div className="px-5 sm:px-7 py-12 text-center flex flex-col items-center">
      <div className="w-14 h-14 rounded-full bg-orange/15 text-orange flex items-center justify-center mb-5">
        <Check size={28} strokeWidth={2.5} />
      </div>
      <h3 className="text-2xl font-extrabold text-charcoal-ink">{t('ct.successTitle')}</h3>
      <p className="mt-3 text-charcoal/75 max-w-md leading-relaxed">{t('ct.successBody')}</p>

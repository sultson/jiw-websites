...
export default function LangToggle({ lang, setLang, compact }: Props) {
  return (
    <div
      className={`inline-flex items-center rounded-full border border-line-cool bg-mist ${compact ? 'p-0.5' : 'p-1'}`}
      role="group"
      aria-label="Language"
    >
      {langs.map((l) => (
        <button
          ...
          className={`${compact ? 'px-2.5 py-1 text-[11px]' : 'px-3 py-1.5 text-xs'} rounded-full font-bold tracking-[0.12em] uppercase transition-colors ${
            lang === l ? 'bg-cobalt text-white' : 'text-ink-mute hover:text-ink'
          }`}
        >
          {l}
        </button>

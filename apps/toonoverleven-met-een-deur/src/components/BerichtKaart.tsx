...
export default function BerichtKaart({
  ...
  return (
    <a
      ...
      className="group flex h-full flex-col overflow-hidden rounded-3xl border border-lijn bg-white transition duration-300 hover:border-teal/40 hover:shadow-lg"
    >
      {bericht.img && (
        <img
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <p className="mt-2.5 line-clamp-3 flex-1 leading-relaxed text-groen/70">
          {schrijfNaam(bericht.samenvatting)}
        </p>
        <span className="mt-5 flex items-center justify-between gap-3">
          <span className="inline-flex items-center gap-2 text-sm font-semibold text-teal-tekst">
            Lees verder
            <ArrowRight className="h-4 w-4 transition group-hover:translate-x-0.5" />
          </span>
          <span className="flex items-center gap-2 text-groen/35">

...
import { DONATIE, Knop, Kop, SPONSORKLIKS, Sectie } from '../ui';
  ...
  return (
    <>
      <Sectie kleur="groen">
        <Kop
          licht
          kicker={teksten.steun.kicker}
          titel={teksten.steun.titel}
          intro={teksten.steun.lead}
        />
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          ...
          <div className="rounded-3xl bg-white p-7 text-groen md:p-8">
            ...
            <div className="mt-6 flex items-center gap-6">
              <img
                ...
                height={144}
              />
              <p className="text-[15px] leading-relaxed text-groen/70">
            ...
            <p className="mt-4 text-sm leading-relaxed text-groen/55">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {teksten.steun.manieren.map((manier, i) => {
              ...
              return (
                ...
                >
                  <h3 className="text-lg">{manier.kop}</h3>
                  <p className="mt-2 flex-1 leading-relaxed text-white/75">{manier.tekst}</p>
                  <a
                    href={href}
                    {...(extern ? { target: '_blank', rel: 'noreferrer' } : {})}
                    className="mt-4 inline-flex items-center gap-1.5 font-semibold text-salie hover:underline"
                  >
                    {LABEL[i] ?? 'Lees verder'}
        ...
        <div className="mt-10 flex flex-col gap-6 rounded-3xl border border-white/15 p-7 md:flex-row md:items-center md:p-8">
          <ShieldCheck className="h-8 w-8 flex-none text-salie" />
          <p className="flex-1 leading-relaxed text-white/75">{teksten.steun.anbi}</p>
          <a
            href="/verantwoording"
            className="inline-flex flex-none items-center gap-2 font-semibold text-salie hover:underline"
          >
      ...
      <Sectie>
              ...
              <li key={s.naam}>
                {s.web ? (
                  <a
                    ...
                    rel="noreferrer"
                    title={s.naam}
                    className="grid h-24 place-items-center rounded-2xl border border-lijn bg-white p-4 transition hover:border-groen/30 hover:shadow-sm"
                  >
                    {beeld}
                  </a>
                ) : (
        ...
        <div className="mt-10 flex flex-wrap items-center gap-x-8 gap-y-4">
          ...
          <p className="max-w-lg text-[15px] text-groen/60">

  ...
  return (
    ...
    >
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="prijsindicatie-modal-title"
      ...
      >
        <div className="flex items-center justify-between gap-4 px-5 md:px-8 py-4 border-b border-line bg-bone sticky top-0 z-10">
          <div>
            <p className="eyebrow">{sectionTitles.prijsindicatie.eyebrow}</p>
            <h2
              id="prijsindicatie-modal-title"
          ...
          <div className="grid gap-8 md:grid-cols-[1fr_1.4fr] px-5 md:px-8 py-6 md:py-8">
            <aside className="hidden md:flex flex-col">
              <p className="text-stone text-base leading-relaxed">
                Wij reageren binnen één werkdag. Vaak komen we langs voor een vrijblijvende opname en geven
                dan een eerlijke prijs.
              </p>
              <ul className="mt-6 space-y-3">
                {usps.slice(0, 4).map((u) => (
                  <li key={u.title} className="flex items-start gap-3">
                    <Check className="mt-0.5 h-5 w-5 flex-shrink-0 text-brick-deep" />

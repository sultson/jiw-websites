...
// Real PNG/ICO icons that must stay PNG. Google's search-result favicon and the
// Apple touch icon are not reliably served as WebP, so keep these untouched.
const keepAsIs = new Set(['favicon-64.png', 'apple-touch-icon.png', 'logo.png', 'og.png']);
...
async function processOne(file: string) {
  ...
  console.log(`${file} ${await human(before)} -> ${await human(after)}`);
}

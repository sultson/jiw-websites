...
// Quality levels mirror Squish's WebP slider (light = higher quality, less compression).
const LIGHT = 82;   // general assets
const HEAVY = 70;   // done-nails photos — reduce a bit more

// Max dimension (longest edge). Photos from phones are often way larger than
// any on-screen use. Capping at 2000px is lossless for our layouts.
const MAX_EDGE = 2000;

const logoMaxEdge = 512; // logo never shown bigger than ~96px

function pickQuality(file: string): number {
  return file.startsWith('done-nails') ? HEAVY : LIGHT;
}
...
async function processOne(file: string) {
  ...
  if (isLogo) {
    // keep as PNG so transparency stays intact, just compress.
  ...
  // Everything else → WebP, matching Squish defaults.
  ...
  console.log(
    `  ${file.padEnd(22)} ${await human(before)} → ${await human(after)}  (WebP q${quality})`,
  );

  // remove the original now that we have a .webp copy

...
import { Sparkles, Heart, Flower2, Leaf, ChevronDown, ArrowRight } from 'lucide-react';
import { serviceCategories } from '../data/services';
import type { Lang } from '../translations';

type Props = { lang: Lang; t: (k: string) => string; onBook: (serviceId?: string) => void };
            ...
            return (
              <div key={cat.id} className="card overflow-hidden">
                ...
                >
                  ...
                  <ChevronDown
                    size={20}
                    className={`shrink-0 text-espresso/60 ${isOpen ? 'rotate-180' : ''}`}
                  />
                </button>
                    ...
                    {cat.services.map(s => (
                      <li key={s.id} className="flex items-center gap-4 px-5 md:px-7 py-4">
                        <div className="flex-1 min-w-0">
                          <p className="text-sm md:text-base font-medium text-espresso truncate">

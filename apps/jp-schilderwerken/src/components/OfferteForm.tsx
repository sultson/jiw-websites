...
export default function OfferteForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const subject = 'Aanvraag gratis prijsindicatie — JP Schilderwerken';
    ...
    <form onSubmit={handleSubmit} noValidate className="space-y-5">
      ...
      <div>
        ...
        <input
          ...
          required
        />
        {errors.email && <p className="mt-1 text-xs text-clay">Vul uw email in.</p>}
      </div>
      ...
      <p className="text-xs text-stone text-center">
        Liever bellen of WhatsAppen?{' '}
        <a
          href={business.phone.href}
          className="font-semibold text-ink underline underline-offset-4 decoration-saffron"
        >

...
const HEAVY = 70;   // gallery and result photos, reduced a bit more

// Max dimension (longest edge). Photos from phones / AI renders are often
// way larger than any on-screen use. Capping at 2000px is lossless for our layouts.
const MAX_EDGE = 2000;

const logoMaxEdge = 512;

function pickQuality(file: string): number {
  return file.startsWith('nails-') || file.includes('result') || file.includes('closeup') ? HEAVY : LIGHT;
}

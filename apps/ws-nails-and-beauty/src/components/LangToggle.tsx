...
export default function LangToggle({ lang, setLang, compact }: Props) {
  return (
    <div
      className={`inline-flex items-center rounded-full border border-plum/15 bg-white/70 backdrop-blur-sm ${compact ? 'p-0.5' : 'p-1'}`}
      role="group"
      aria-label="Language"
    >
      {langs.map(l => (
        <button
          ...
          className={`${compact ? 'px-2.5 py-1 text-[11px]' : 'px-3 py-1.5 text-xs'} rounded-full font-medium tracking-wider uppercase ${
            lang === l ? 'bg-plum text-porcelain' : 'text-plum/60 hover:text-plum'
          }`}
        >
          {l}
        </button>

...
export default function Terms({ t }: Props) {
  ...
  return (
    <section id="voorwaarden" className="py-20 md:py-28 bg-blush-soft/50">
      <div className="max-w-3xl mx-auto px-5 sm:px-8">
        <div className="text-center mb-10">
          <span className="kicker">{t('terms.kicker')}</span>
          <h2 className="mt-3 font-serif text-4xl md:text-5xl">{t('terms.title')}</h2>
          ...
          {items.map(i => {
            ...
            return (
              <div key={i}>
                ...
                {isOpen && (
                  <p className="pb-6 pl-9 pr-2 text-espresso/75 leading-relaxed text-sm">
                    {t(`terms.a${i}`)}
                  </p>
                )}
              </div>
        ...
        <div className="mt-8 text-center">
          <a
            ...
            className="inline-flex items-center gap-2 text-sm text-gold hover:underline underline-offset-4"
          >
            {t('terms.full')} <ArrowRight size={14} />
          </a>
        </div>
      </div>
    </section>

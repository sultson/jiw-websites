...
const onderdelen: PortableTextComponents = {
  block: {
    normal: ({ children }) => <p className="mt-5 leading-relaxed text-inkt/80">{children}</p>,
    ...
    blockquote: ({ children }) => (
      <blockquote className="mt-6 border-l-2 border-salie-diep pl-5 text-lg italic text-inkt/75">
        {children}
      </blockquote>
    ),
  },
  ...
  listItem: {
    bullet: ({ children }) => (
      <li className="flex items-start gap-2.5 leading-relaxed text-inkt/80">
        <span className="mt-2.5 h-1.5 w-1.5 flex-none rounded-full bg-salie-diep" />
        <span>{children}</span>
      </li>
    ),
    number: ({ children }) => <li className="leading-relaxed text-inkt/80">{children}</li>,
  },
  marks: {
    strong: ({ children }) => <strong className="font-semibold text-inkt">{children}</strong>,
    link: ({ children, value }) => {
      ...
      return (
        <a
          ...
          className="font-semibold text-wijn underline underline-offset-2 hover:text-inkt"
        >
          {children}
        </a>
      );
    ...
    image: ({ value }) => {
      ...
      return (
        <figure className="mt-8">
          ...
          />
          {value.bijschrift && (
            <figcaption className="mt-2.5 text-sm text-inkt/55">{value.bijschrift}</figcaption>
          )}
        </figure>
      );
    },

...
export default function Verantwoording({ teksten }: { teksten: Teksten }) {
  return (
    <>
    <PaginaHero
      kruimels={[{ label: 'Verantwoording en ANBI' }]}
      titel={teksten.verantwoording.titel}
      lead={teksten.verantwoording.lead}
      smal
    />
    ...
    <Sectie>
              ...
              <li key={d.bestand}>
                ...
                >
                  <span className="mt-0.5 grid h-10 w-10 flex-none place-items-center rounded-full bg-salie/45 transition group-hover:bg-salie">
                    <Download className="h-4 w-4 text-inkt" />
                  </span>
                  <span className="flex-1">
                    <span className="block font-semibold group-hover:underline">{d.naam}</span>
                    <span className="mt-0.5 block text-[15px] text-inkt/65">{d.over}</span>
                  </span>
                  <span className="mt-1 flex-none text-sm text-inkt/45">PDF, {d.grootte}</span>
                </a>
              </li>
            ))}
          </ul>
        ...
        <div className="grid gap-6">
          <div className="rounded-3xl border border-lijn bg-white p-7 md:p-8">
            ...
            <dl className="mt-5 space-y-3 text-[15px]">
              ...
              <Gegeven kop="Contact">
                <a href={TEL_LINK} className="text-wijn hover:underline">
                  {TEL}
                </a>
                {' · '}
                <a href={`mailto:${MAIL}`} className="text-wijn hover:underline">
                  {MAIL}
                </a>
              </Gegeven>
          ...
          <div className="rounded-3xl border border-lijn bg-white p-7 md:p-8">
            ...
            <p className="mt-3 leading-relaxed text-inkt/75">{teksten.verantwoording.beloning}</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="rounded-3xl border border-lijn bg-white p-7 md:p-8">
          ...
          <p className="mt-3 leading-relaxed text-inkt/75">{teksten.verantwoording.doel}</p>
        </div>
        ...
        <div className="rounded-3xl border border-lijn bg-white p-7 md:p-8">
          ...
          <div className="mt-5 grid gap-6 sm:grid-cols-2">
            <div>
              <h3 className="text-xs font-bold uppercase tracking-[0.16em] text-wijn">
                Bestuur
              </h3>
              <ul className="mt-3 space-y-2">
                {teksten.verantwoording.bestuur.map((b) => (
                  <li key={b.naam}>
                    ...
                    <span className="block text-[15px] text-inkt/60">{b.rol}</span>
                  </li>
                ))}
              </ul>
            </div>
              ...
              <ul className="mt-3 space-y-2">
                {teksten.verantwoording.advies.map((a) => (
                  <li key={a.naam}>
                    ...
                    <span className="block text-[15px] text-inkt/60">{a.rol}</span>
                  </li>
                ))}
              </ul>
            </div>
...
function Gegeven({ kop, children }: { kop: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-wrap gap-x-3">
      ...
      <dd className="text-inkt/75">{children}</dd>
    </div>
  );
}

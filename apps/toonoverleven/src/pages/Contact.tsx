...
import { ArrowUpRight, CalendarDays, DoorOpen, Mail, MapPin, Phone, Users, Wallet } from 'lucide-react';
...
export default function Contact({ teksten }: { teksten: Teksten }) {
  return (
    <>
      <PaginaHero
        kruimels={[{ label: teksten.contact.kicker }]}
        titel={teksten.contact.titel}
        lead={teksten.contact.lead}
        foto={{
            ...
            <dl className="mt-8 space-y-5">
              ...
              <Regel icoon={CalendarDays} kop="Inloopmomenten">
                {teksten.contact.openingstijden.split('\n').map((regel, i) => (
                  <span key={regel} className={i === 2 ? 'block text-inkt/55' : 'block'}>
                    {regel}
                  </span>
                ))}
              </Regel>
      ...
      <Sectie kleur="room-diep" id="bericht" className="scroll-mt-20">
        <div className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:gap-14">
          <div>
            <Kop titel={teksten.contact.formulierTitel} klein />
            <p className="mt-4 leading-relaxed text-inkt/70">{teksten.contact.formulierTekst}</p>
          </div>
          <Formulier />
        </div>
      </Sectie>
...
function Regel({
  ...
  return (
    <div className="flex gap-3.5">
      <Icoon className="mt-1 h-5 w-5 flex-none text-wijn" />
      <div>
        <dt className="font-semibold">{kop}</dt>
        <dd className="mt-0.5 leading-relaxed text-inkt/75">{children}</dd>
      </div>
    </div>
  );
}
